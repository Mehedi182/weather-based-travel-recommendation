...
class BestDistrictsView(APIView):
    def get(self, request):
        # Get the most recent data (last 7 days)
        date_threshold = date.today() - timedelta(days=7)

        # Get unique districts with their average values
        districts = (
...
class TravelRecommendationView(APIView):
    def post(self, request):
        ...
        try:
            ...
            dest_data = DistrictWeatherData.objects.filter(
                name=destination_district, date=travel_date
            ).first()
            ...
            # Get comparable current location data (nearest district)
            # current_district = self.find_nearest_district(current_lat, current_lon)
    ...
    def generate_recommendation(self, temp_diff, pm25_diff):
        if temp_diff <= 0 and pm25_diff <= 0:
            ...
            # pm25_verb = "slightly" if abs(pm25_diff) < 10 else "significantly"
            ...
            # if pm25_diff >= 0:
            #     reasons.append("has worse air quality")

            reason = "our destination is hotter and has worse air quality than your current location. It’s better to stay where you are."
